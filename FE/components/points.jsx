...
export default function Points() {
  ...
  const load = async () => {
    try {
      const [pRes, cRes] = await Promise.all([API.get("/points"), API.get("/customers")])
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm muted mt-1">Quản lý điểm tích lũy, đổi thưởng và hết hạn</p>
      </div>
        ...
        <div className="flex items-end gap-3">
          ...
          <div className="w-32">
            ...
            <input
              placeholder="0"
              value={amount || ""}
              onChange={(e) => setAmount(e.target.value ? Number.parseInt(e.target.value, 10) || 0 : 0)}
        ...
        <table className="min-w-full text-sm">
          <thead>
            <tr className="border-b border-dark-border/50">
              ...
              <th className="px-4 py-3 text-center font-semibold text-primary">Loại</th>
              <th className="px-4 py-3 text-center font-semibold text-primary">Điểm</th>
              <th className="px-4 py-3 text-left font-semibold text-primary">Thời gian</th>
            </tr>
          </thead>
          <tbody>
            {(items || []).map((it) => (
              <tr key={it._id} className="border-b border-dark-border/30 hover:bg-primary/5 transition-colors">
                <td className="px-4 py-3 font-mono text-xs text-text-secondary">{it._id.slice(-6)}</td>
                ...
                <td className="px-4 py-3 text-center font-semibold">{it.points}</td>
                <td className="px-4 py-3 text-sm muted">
                  {it.occurredAt ? new Date(it.occurredAt).toLocaleString("vi-VN") : "N/A"}
                </td>
              </tr>
            ))}
          </tbody>
